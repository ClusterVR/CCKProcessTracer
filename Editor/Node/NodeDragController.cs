...
{
    public static class NodeDragController
    {
        public static ProcessObject draggingObject { get; private set; }
        ...
        public static void Control()
        {
            ...
            {
                ProcessObject foundObject = null;
                for (int i = ObjectFrameDrawer.objectFrames.Count - 1; i >= 0; i--)
                {
                    var frame = ObjectFrameDrawer.objectFrames[i];
                    if (frame == null || frame.processObject == null) continue;
                    if (!KeyFilter.IsFrameVisible(frame.processObject)) continue;

                    if (frame.nameButtonRect.Contains(canvasMousePos))
                    {
                        foundObject = frame.processObject;
                        break;
                    }
                }
            ...
            else if (e.type == EventType.MouseDrag && e.button == 0)
            {
                if (draggingObject != null)
                {
                    ...
                    // ドラッグ中にリアルタイムに位置と描画オブジェクト（接続線など）を更新する
